...
int test_number_creation() {
    ...
    // Verify values
    test_assert(fabs(n->floated - 3.14) < 0.0001, "number_create sets correct value");
    test_assert(i->integer == 42, "integer_create sets correct value");
    test_assert(fabs(f->floated - 2.718) < 0.0001, "float_create sets correct value");
    test_assert(fabs(d->doubled - 1.618) < 0.0001, "double_create sets correct value");
...
int test_vector_reference_counting() {
    ...
    test_assert(v1->number.ref_count == 1, "Vector initial reference count is 1");
    test_assert(v2->number.ref_count == 1, "Vector initial reference count is 1");
    
    // Test reference counting in vector operations
    vector *result = vector_addition(
                       vector_multiplication(
                         number_ref(v1),  // Ref v1 since we're using it multiple times
                         float_create(2.0)
                       ),
                       (number *)v2
                     );
    
    // v1 should still exist with ref_count = 1 (original - 1 + multiplication)
    test_assert(v1->number.ref_count == 1, "v1 reference count is correct after operations");
    
    // v2 should be unreferenced by vector_addition
    
    // result should be the same as v1 (operations modify in place)
    test_assert(result == v1, "Result is the same as v1 (operations modify in place)");
